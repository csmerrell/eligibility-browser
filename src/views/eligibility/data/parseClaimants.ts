...
export const getClaimants = () => {
   ...
   * */
  return fetch(`${currUrl.protocol}//${currUrl.hostname}${currUrl.port ? ':' + currUrl.port : ''}/api-data.json`)
    .then(response => {
    ...
    .catch(async error => {
      console.log("Fetch failed to find the sample data file. Importing it directly as a fallback");
      let claimants;
